...
async def update_handler(client: Client, message):
    ...
    try:
        ...
        commit_info = latest_commit.stdout.strip()
...
async def configure(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                "Mute ✅" if penalty == "mute" else "Mute",
                callback_data="mute"
            ),
            InlineKeyboardButton(
                "Ban ✅" if penalty == "ban" else "Ban",
                callback_data="ban"
            )
        ],
        [
...
async def command_free(client: Client, message):
    ...
    if message.reply_to_message:
        ...
        return await client.send_message(
            chat_id,
            "**Reply or use /free user or id to whitelist someone.**"
        )
    ...
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "🚫 Unwhitelist",
                callback_data=f"unwhitelist_{target.id}"
            ),
            InlineKeyboardButton(
                "🗑️ Close",
                callback_data="close"
...
async def command_unfree(client: Client, message):
    ...
    if message.reply_to_message:
        ...
        return await client.send_message(
            chat_id,
            "**Reply or use /unfree user or id to unwhitelist someone.**"
        )
    ...
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "✅ Whitelist",
                callback_data=f"whitelist_{target.id}"
            ),
            InlineKeyboardButton(
                "🗑️ Close",
                callback_data="close"
...
async def command_freelist(client: Client, message):
    ...
    if not ids:
        await client.send_message(
            chat_id,
            "**⚠️ No users are whitelisted in this group.**"
        )
        return
...
async def callback_handler(client: Client, callback_query):
    ...
    if data == "warn":
        ...
        kb = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
                    f"3 ✅" if selected_limit == 3 else "3",
                    callback_data="warn_3"
                ),
                InlineKeyboardButton(
                    f"4 ✅" if selected_limit == 4 else "4",
                    callback_data="warn_4"
                ),
                InlineKeyboardButton(
                    f"5 ✅" if selected_limit == 5 else "5",
        ...
        return await callback_query.message.edit_text(
            "**Select number of warns before penalty:**",
            reply_markup=kb
        )
    ...
    if data.startswith("warn_"):
        ...
        kb = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
                    f"3 ✅" if count == 3 else "3",
                    callback_data="warn_3"
                ),
                InlineKeyboardButton(
                    f"4 ✅" if count == 4 else "4",
                    callback_data="warn_4"
                ),
                InlineKeyboardButton(
                    f"5 ✅" if count == 5 else "5",
                    callback_data="warn_5"
                )
            ],
            [
        ...
        try:
            ...
            kb = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(
                        "Whitelist ✅",
                        callback_data=f"whitelist_{target_id}"
                    ),
                    InlineKeyboardButton(
                        "🗑️ Close",
                        callback_data="close"
            ...
            await callback_query.message.edit_text(
                f"I don't have permission to {action} users."
            )
    ...
    if data.startswith("cancel_warn_"):
        ...
        mention = (
            f"[{full_name}](tg://user?id={target_id})"
        )
        ...
        await callback_query.message.edit_text(
            f"**✅ {mention} [`{target_id}`] has no more warnings!**",
            reply_markup=kb
        )
    ...
    if data.startswith("whitelist_"):
        ...
        kb = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
                    "🚫 Unwhitelist",
                    callback_data=f"unwhitelist_{target_id}"
                ),
                InlineKeyboardButton(
                    "🗑️ Close",
                    callback_data="close"
        ...
        await callback_query.message.edit_text(
            f"**✅ {mention} [`{target_id}`] has been whitelisted!**",
            reply_markup=kb
        )
    ...
    if data.startswith("unwhitelist_"):
        ...
        await callback_query.message.edit_text(
            f"**❌ {mention} [`{target_id}`] has been removed from whitelist.**",
            reply_markup=kb
        )
    ...
    if URL_PATTERN.search(bio):
        ...
        if mode == "warn":
                ...
                try:
                    if penalty == "mute":
                        ...
                        await sent.edit_text(
                            f"**{full_name} has been 🔇 muted for [Link In Bio].**",
                            reply_markup=kb
                        )
                        ...
                        await sent.edit_text(
                            f"**{full_name} has been 🔨 banned for [Link In Bio].**",
                            reply_markup=kb
                        )
            ...
            try:
                ...
                return await message.reply_text(
                    f"I don't have permission to {mode} users."
                )
